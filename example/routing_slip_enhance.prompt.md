...
For more examples, see the `/example` directory in the repository.
 ...
 I made routing-slip.config.ts the single source of truth by:

  1. Modified RoutingSlipModule.forRoot() to read the mode from the registry instead of accepting it as a parameter:
  // lib/routing-slip.module.ts
  const executionMode = RoutingSlipModeRegistry.getMode();  // Read from registry
  const queuePrefix = options.queuePrefix || RoutingSlipModeRegistry.getQueuePrefix();

  2. Updated routing-slip.config.ts to Distributed mode:
  RoutingSlipModeRegistry.setMode(RoutingSlipExecutionMode.Distributed, 'myapp');

  3. Simplified app.module.ts - no longer needs executionMode or queuePrefix:
  RoutingSlipModule.forRoot({
    enableEventSubscribers: true
  }),
